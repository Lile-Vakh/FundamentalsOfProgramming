...
public class ScrewStore {
    ...
    public void addItem(Screw type, int amount) {
        ...
        if(current == null) stock.put(type, amount);
        else stock.put(type, amount + current);
    }

    public void takeOrder(Screw type, int amount) {
        if(amount > 0) orders.add(new Order(type, amount));
    }
    ...
    public int count() {
        int count = 0;
        for(Integer i : stock.values()) count += i;
        return count;
    }
    ...
    public double value() {
        ...
        for(Entry<Screw, Integer> entry : stock.entrySet())
            val += entry.getKey().getPrice() * entry.getValue();
        return val;
    }
